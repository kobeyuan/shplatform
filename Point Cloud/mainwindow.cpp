...
void MainWindow::initConnect()
{
	...
	//--------kinect
	connect(ui.actionKinect_Show,SIGNAL(triggered()),this,SLOT(displayKinect()));
	connect(ui.actionRigister,SIGNAL(triggered()),this,SLOT(showRigisterDlg()));
	//connect(ui.actionR)
	//
...
void MainWindow::showNormalDlg()
{
	...
	//paraDlg_Normal->setFeatures(StdParaDlg::NoDockWidgetFeatures);
}
...
void MainWindow::showRigisterDlg()
{
	if(m_paraDlgRigister != 0)
	{
		m_paraDlgRigister->close();
		delete m_paraDlgRigister;
	}
	m_paraDlgRigister = new StdParaDlg(paras,area,this);
	m_paraDlgRigister->setAllowedAreas(Qt::LeftDockWidgetArea
		| Qt::RightDockWidgetArea);
	addDockWidget(Qt::LeftDockWidgetArea,m_paraDlgRigister);
	m_paraDlgRigister->setFloating(false);
	m_paraDlgRigister->hide();
	m_paraDlgRigister->showRigisterParaDlg();
}
...
void MainWindow::displayKinect()
{
	//area->dataMgr.loadPlyToOriginal("pointC_kinect.ply");//需要先读取一个模型
	area->m_kinect->openKinect();
	area->dataMgr.loadXYZRGB(area->m_kinect->m_pointCloud);
	area->initAfterOpenFile();
	area->updateGL();	
}

...
void DataMgr::loadXYZN(QString fileName)
{
 ...
 void DataMgr::loadXYZRGB(const vector<SColorPoint3D> & pCloudFKnt)
 {
 	//clearCMesh(samples);
	 clearCMesh(original);
 	if(pCloudFKnt.empty())
 	{
 		std::cout<<"point cloud is empty"<<endl;
 		return;
 	}
    int verIndex = 0;
 	for (int i = 0; i < pCloudFKnt.size();i ++/*= 100*/)
 	{
 		CVertex v;
 		//
 		v.P()[0] = pCloudFKnt.at(i).X;
 		v.P()[1] = pCloudFKnt.at(i).Y;
 		v.P()[2] = pCloudFKnt.at(i).Z;
 		//
 		v.N()[0] = pCloudFKnt.at(i).R;
 		v.N()[1] = pCloudFKnt.at(i).G;
 		v.N()[2] = pCloudFKnt.at(i).B;
 		//
 		v.m_index = verIndex;
 		//samples.vert.push_back(v);
 		//samples.bbox.Add(v.P());
...
/*
*/
// void DataMgr::setSample(CMesh & samp)
// {
// 	 samples(samp);
// }

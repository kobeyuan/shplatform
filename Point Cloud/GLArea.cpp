...
GLArea::GLArea(QWidget *parent): QGLWidget(/*QGLFormat(QGL::DoubleBuffer | QGL::DepthBuffer |QGL::SampleBuffers),*/ parent),
	               para(global_paraMgr.getGlareaParameterSet()),
								 ...
								 m_rigister(global_paraMgr.getRigisterParameterSet())
{
	...
	cout<<"construct kinect"<<endl;
	m_kinect = new KinectShow(this);
...
void GLArea::paintGL() 
{
	...
	vcg::Point3f lightPos = para->getPoint3f("Light Position");
	float lpos[4];
	lpos[0] = lightPos[0];
	lpos[1] = lightPos[1];
	lpos[2] = lightPos[2];
	...
	View<float> view;
	view.GetView();
	vcg::Point3f viewpoint = view.ViewPoint();
...
{
	dataMgr.loadPlyToSample("default_original.ply");//default.ply
	dataMgr.loadPlyToOriginal("721Me.ply");//.ply
	//dataMgr.loadPlyToOriginal("pointC_kinect.ply");
	initAfterOpenFile();
	updateGL();
}
...
void GLArea::runCloudMap()
{
	...
	runPointCloudAlgorithm(m_rigister);
...
{
	vector<vcg::Point3f> RGB_normals;
	...
	if(RGB_counter == 3)
	{
		...
		RGB_normals.assign(3, vcg::Point3f(0.0, 0.0, 0.0));
...
void GLArea::readRGBNormal(QString fileName)
{
	...
	vector<vcg::Point3f> rgb_normal(3, vcg::Point3f(0, 0, 0));

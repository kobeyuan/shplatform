...
public:
	...
	vcg::Point3f eigen_vector0; //Associate with the biggest eigen value
	vcg::Point3f eigen_vector1; // Also use for remember last better virtual point
...
public:
	operator vcg::Point3f &()
	{
		return P();
	}

	operator const vcg::Point3f &() const
	{
		return cP();
	}
    ...
    is_fixed_original(false),
		eigen_vector0(vcg::Point3f(1, 0, 0)),
		eigen_vector1(vcg::Point3f(0, 1, 0)),
    skel_radius(-1.0)
		{
			N() = vcg::Point3f(0,0,0);
	...
	void remove() //important, some time we don't want to earse points, just remove them
	{
		...
		is_skel_ignore = true;
		P() = vcg::Point3f(88888888888.8, 88888888888.8, 88888888888.8);
	}
	...
	void recompute_m_render()
	{
		...
		vcg::Point3f normal = N();
		normal.Normalize();

		vcg::Point3f helper(x/1000.0, y/1000.0, z/1000.0);
		vcg::Point3f new_m3_to_m5 = normal ^ helper;
		new_m3_to_m5.Normalize();
		vcg::Point3f new_m6_to_m8 = normal ^ new_m3_to_m5;

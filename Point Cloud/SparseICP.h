...
namespace SparseICP
{
	namespace Mynanoflann 
	{
		...
		template <typename MatrixType, int DIM = -1, class Distance = nanoflann::metric_L2, typename IndexType = int>//class 改成了typename
		struct KDTreeAdaptor
		{
			typedef KDTreeAdaptor<MatrixType,DIM,Distance> self_t;
			typedef typename MatrixType::Scalar              num_t;
			typedef typename  Distance::template traits<num_t,self_t>::distance_t metric_t;
			typedef typename nanoflann::KDTreeSingleIndexAdaptor< metric_t,self_t,DIM,IndexType>  index_t;//加上了域名 + typename 
			index_t * index;
			KDTreeAdaptor(const MatrixType &mat, const int leaf_max_size = 10) : m_data_matrix(mat)//构造函数
			{
				const size_t dims = mat.rows();                                                    //kd-tree的维数？
				index = new index_t( dims, *this, nanoflann::KDTreeSingleIndexAdaptorParams(leaf_max_size, dims ) );
				index->buildIndex();
			}
			~KDTreeAdaptor() {delete index;}
			const MatrixType &m_data_matrix;                                                       //类的数据成员
...
#pragma omp parallel for
		...
		{
			/// Build kd-tree
			Mynanoflann::KDTreeAdaptor<Eigen::MatrixBase<Derived2>, 3, nanoflann::metric_L2_Simple> kdtree(Y);
			/// Buffers
			...
			//verMap.Zero(X.cols());
			/// ICP
			for(int icp=0; icp<par.max_icp; ++icp) //整个点云的迭代
			{
				/// Find closest point
#pragma omp parallel for
				for(int i=0; i<X.cols(); ++i) //Step One
				{
					unsigned int mp = kdtree.closest(X.col(i).data());
					Q.col(i) = Y.col(mp/*kdtree.closest(X.col(i).data())*/);//从Y中找出离Ｘ的第i个点最近的点，赋给Q，找对应！
					verMap(i) = mp;
				}
				//cout<<"点对应矩阵"<<Q<<endl;
				/// Computer rotation and translation----Step Two
				double mu = par.mu;
				for(int outer=0; outer<par.max_outer; ++outer)
				{
					...
					{
						/// Z update (shrinkage)
						Z = X-Q+C/mu;                      //对应点的残差;Q == X的对应点
						shrink<3>(Z, mu, par.p);          // 更新了残差 ----- Step  2.1
					...
					if(!par.use_penalty) C.noalias() += mu*P;//更新栏目他-----Step 2.3
					/// mu update (penalty)

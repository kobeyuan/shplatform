...
{
	vector<int> nCard(Max, 0);//Creates a vector with Max copies of 0.
	srand(time(NULL));
	for(int i=0; i < Max; i++)
	{
		nCard[i] = i;
	}
	random_shuffle(nCard.begin(), nCard.begin() + Max);//Rearranges the elements in the range [first,last) randomly
